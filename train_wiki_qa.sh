#!/bin/bash
#SBATCH --job-name=t5_base_wiki_qa
#SBATCH --gpus=ampere:1
#SBATCH --mem=64G
#SBATCH --cpus-per-task=32

# Script to train T5-base on WikiQA dataset
# Trains for 75,000 steps as per train_tips.txt
# Saves checkpoint in: models/wiki_qa/best.pt
...
# Train T5-base on WikiQA dataset
# inference_dataset_mixture=None ensures evaluation only on WikiQA (same as train_dataset)
python src/training.py \
    -c configs/t5_base.json \
    -k \
    train_dataset=wiki_qa \
    train_dataset_mixture=None \
    inference_dataset_mixture=None \
    project_name=t5_finetuning \
    experiment_name=t5-base-wiki_qa \
    num_batches=75000

echo "Training completed for WikiQA dataset"
