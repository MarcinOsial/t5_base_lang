...
# Optional import for LinearizedTaskVector (only needed for vision models)
try:
    from src.linearize import LinearizedImageEncoder
except ImportError:
    LinearizedImageEncoder = None
    pass
...
class _TaskVector(abc.ABC):
    def __init__(
        ...
        Args:
            ...
            finetuned_checkpoint: Path to the finetuned model checkpoint.
            vector: Dictionary containing the task vector directly.
            use_half: Whether to convert the task vector to half precision (float16).
                      Setting to False keeps vectors in full precision (float32).
        """
        if vector is not None:
            ...
            with torch.no_grad():
                ...
                for key in pretrained_state_dict:
                    ...
                    # Convert to 16-bit precision if use_half is True, otherwise keep as float32
                    self.vector[key] = diff.half() if use_half else diff
...
class NonLinearTaskVector(_TaskVector):
    ...
    def __init__(
        ...
        """Initialize a NonLinearTaskVector with optional half precision control.
...
class LinearizedTaskVector(_TaskVector):
    ...
    def __init__(
        ...
        """Initialize a LinearizedTaskVector with optional half precision control.
...
def nonlinear_to_linear(nonlinear_task_vector, use_half=True):
    ...
    if isinstance(nonlinear_task_vector, LinearizedTaskVector):
        ...
        return LinearizedTaskVector(vector=linear_params, use_half=use_half)


def linear_to_nonlinear(linear_task_vector, param_names, use_half=True):
